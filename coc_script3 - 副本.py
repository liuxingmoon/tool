...
def play(wait_time,skipids):
    ...
    action = r'"E:\Program Files\DundiEmu\DunDiEmu.exe" -multi %d -disable_audio  -fps 40' %(startid)
...
def play_donate(donateid,*args):
    ...
    action = r'"E:\Program Files\DundiEmu\DunDiEmu.exe" -multi %d -disable_audio  -fps 40' %(int(donateid_now))
...
def instance(donate_switch,donateid):
    ...
    if now_time >= day_time_end or now_time < night_time_end:
        ...
        if donate_status not in ['True','1','T']:#切换为打资源状态
        ...
        if donate_status in ['True','1','T']:#切换为捐兵状态
